#!/bin/zsh
# Generate an MP3 of the selection and save it to the configured folder.
# Text over MAX_CHUNK characters is generated in pieces and joined with ffmpeg.
...
RAW_FOLDER="${POPCLIP_OPTION_SAVE_FOLDER:-~/Library/Mobile Documents/com~apple~CloudDocs/TTS_Inbox}"
SAVE_FOLDER="${RAW_FOLDER/#\~/$HOME}"
mkdir -p "$SAVE_FOLDER"
OUTPUT="$SAVE_FOLDER/TTS_$(date +%Y%m%d-%H%M%S).mp3"

if [[ $TEXT_LEN -le $MAX_CHUNK ]]; then
  if ! generate "$TEXT" "$OUTPUT"; then
    rm -f "$OUTPUT"
    alert "Audio request failed. Check your keys and quota."
    exit 1
  fi
else
  if ! command -v "$FFMPEG_BIN" >/dev/null 2>&1; then
    alert "Text is over $MAX_CHUNK characters and ffmpeg is not installed. Install it with: brew install ffmpeg"
    exit 1
  fi
  WORK="/tmp/popclip_tts_save_$$"
  mkdir -p "$WORK"
  ...
  for chunk in "${CHUNKS[@]}"; do
    part="$WORK/part_$idx.mp3"
    if ! generate "$chunk" "$part"; then
  ...
  "$FFMPEG_BIN" -f concat -safe 0 -i "$LIST" -c copy -y "$OUTPUT" 2>/dev/null
fi

if [[ ! -s "$OUTPUT" ]]; then
  alert "Output file is empty. Generation may have failed."
  exit 1
fi

FNAME=$(basename "$OUTPUT")
osascript -e "display notification \"Saved $FNAME\" with title \"ElevenLabs TTS\"" 2>/dev/null
echo "$OUTPUT"
